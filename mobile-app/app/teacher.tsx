  ...
  useEffect(() => {
    ...
    getStudentSession()
      .then(async (session) => {
        ...
        const [summary, notifications] = await Promise.all([summaryResponse.json(), notificationsResponse.json()]);
        setState({
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#07111f" }}>
      <ScrollView contentContainerStyle={{ padding: 20, gap: 16 }}>
        ...
        <Text style={{ color: "#9fb0c7", fontSize: 15 }}>Teaching load, next class, and faculty communication</Text>
        ...
        <View style={{ flexDirection: "row", gap: 12 }}>
          <AnimatedSurface
            from={{ opacity: 0, translateY: 12 }}
            animate={{ opacity: 1, translateY: 0 }}
            transition={{ delay: 80, type: "timing", duration: 450 }}
            style={{ flex: 1, borderRadius: 22, padding: 18, backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" }}
          >
            <Text style={{ color: "#dbeafe", fontSize: 14 }}>Courses</Text>
            <Text style={{ color: "white", marginTop: 8, fontSize: 22, fontWeight: "700" }}>{state.totalCourses}</Text>
          </AnimatedSurface>
          <AnimatedSurface
            from={{ opacity: 0, translateY: 12 }}
            animate={{ opacity: 1, translateY: 0 }}
            transition={{ delay: 140, type: "timing", duration: 450 }}
            style={{ flex: 1, borderRadius: 22, padding: 18, backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" }}
          >
            <Text style={{ color: "#dbeafe", fontSize: 14 }}>Teaching Load</Text>
            <Text style={{ color: "white", marginTop: 8, fontSize: 22, fontWeight: "700" }}>{state.teachingLoad}</Text>
          </AnimatedSurface>
        </View>
