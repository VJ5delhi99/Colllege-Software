...
- Product-integrated MVP: `98%`
- Production-ready ERP platform: `81%`
...
| Finance | 84% | Payments, reconciliation, student-facing summaries, provider readiness reporting, rollout validation, student-initiated payment-session flows, and local payment completion now exist, but real live-provider registration and full external checkout/webhook rollout are still maturing. |
...
| Automated testing | 77% | Build and smoke verification are healthy, and coverage now includes request fulfillment, delivered-document metrics, plus teacher grading-state calculations, but broad regression and end-to-end coverage remain limited. |
